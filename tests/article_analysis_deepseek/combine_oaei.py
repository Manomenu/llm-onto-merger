#!/usr/bin/env python3
"""Combine N per-turn OAEI-rejection CSVs into a median/min/max table + chart.
...
this script stacks the per-turn values and reports the median, min, and max
across turns.  A method/dataset cell that is missing in a turn (empty string
— e.g. no scenario_3 output for that turn) is simply skipped for that turn
rather than treated as zero.

Two outputs:
  --pm-output <table.csv>   thesis-friendly table:
        method,dataset,reference_total,aml_total,rejected_correct (median [min; max]),accepted_aml_fp (median [min; max])
  --jpg-output <chart.jpg>  grouped bar chart mirroring oaei_rejection.py's
        ...
        asymmetric min/max error bars across turns.
"""
...
def main() -> None:
    ...
    ap.add_argument("--pm-output", required=True, type=Path,
                    help="'median [min; max]' thesis table CSV")
    ap.add_argument("--jpg-output", required=True, type=Path,
                    help="grouped bar chart with min/max error bars")
    ...
    median: dict[tuple[str, str, str], float] = {}
    vmin: dict[tuple[str, str, str], float] = {}
    vmax: dict[tuple[str, str, str], float] = {}
    ...
    with args.pm_output.open("w", newline="") as fh:
        ...
        w.writerow(["method", "dataset", "reference_total", "aml_total",
                    "rejected_correct (median [min; max])", "accepted_aml_fp (median [min; max])"])
        for dataset in dataset_order:
            for method in method_order:
                ...
                for measure, _ in _MEASURES:
                    ...
                    if mkey not in median:
                        row.append("n/a")
                        continue
                    md, lo, hi = median[mkey], vmin[mkey], vmax[mkey]
                    row.append(f"{md:.{args.decimals}f} [{lo:.{args.decimals}f}; {hi:.{args.decimals}f}]")
                w.writerow(row)
    print(f"wrote {args.pm_output}")

    # ── grouped bar chart with min/max error bars (rows=datasets, cols=measures) ──
    ...
    for di, dataset in enumerate(dataset_order):
        for mi, (measure, title) in enumerate(_MEASURES):
            ...
            for method in method_order:
                ...
                vals.append(median.get(key, float("nan")))
                los.append(vmin.get(key, float("nan")))
                his.append(vmax.get(key, float("nan")))
            ...
            bars = ax.bar(method_order, plot_vals, yerr=[err_lo, err_hi], color=_BAR_COLOR,
                          edgecolor="black", linewidth=0.5, error_kw=_ERR_KW)
            labels = []
            for v, lo, hi in zip(vals, los, his):
                if v != v:
                    labels.append("n/a")
                elif hi == hi and lo == lo and hi > lo:
                    labels.append(f"{v:.{args.decimals}f}\n{lo:.{args.decimals}f}–{hi:.{args.decimals}f}")
                else:
                    labels.append(f"{v:.{args.decimals}f}")
    ...
    subtitle = f"{args.title}  (median (min–max) over {n_turns} runs)"
