...
public class AssignBlockProperties {
	public HModel run(HModel ontM) {
		long startTime = System.currentTimeMillis();
		ArrayList<BlockModel> clusterList = ontM.getClusters();
	...
	private static HModel ObjectPropertyRangeAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		if (rang instanceof OWLClassImpl) {
			...
			int clusterId = BlockShareFunc.findClusterOfClass(eqRang, ontM);
			if (clusterId != -1) {
			...
			while (rangExist.hasNext()) {
				...
				if (ex instanceof OWLClass) {
					...
					int clusterId = BlockShareFunc.findClusterOfClass(eq, ontM);
					if (clusterId != -1) {
						ClusterManager = ontM.getClusters().get(clusterId).getManager();
						ClusterOntology = ontM.getClusters().get(clusterId).getOntology();
						ClusterManager.addAxiom(ClusterOntology, myAxiom);
	...
	private static HModel SymmetricObjectPropertyAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		int clusterId = BlockShareFunc.findClusterOfPro(subProperty.asOWLObjectProperty(), ontM);
		if (clusterId != -1) {
			ClusterManager = ontM.getClusters().get(clusterId).getManager();
			ClusterOntology = ontM.getClusters().get(clusterId).getOntology();
			ClusterManager.addAxiom(ClusterOntology, myAxiom);
	...
	private static HModel FunctionalDataPropertyAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		int clusterId = BlockShareFunc.findClusterOfDro(subProperty.asOWLDataProperty(), ontM);
		if (clusterId != -1) {
			ClusterManager = ontM.getClusters().get(clusterId).getManager();
			ClusterOntology = ontM.getClusters().get(clusterId).getOntology();
			ClusterManager.addAxiom(ClusterOntology, myAxiom);
	...
	private static HModel AnnotationAssertionAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		if (subject instanceof IRI) {
			...
			if (cl != null) {
				...
				int clusterId = BlockShareFunc.findClusterOfClass(eqCls, ontM);
				...
				if (clusterId != -1) {
					...
					clusterId = BlockShareFunc.findClusterOfPro(proNew, ontM);
					if (clusterId != -1) {
						ClusterManager = ontM.getClusters().get(clusterId).getManager();
						ClusterOntology = ontM.getClusters().get(clusterId).getOntology();
						ClusterManager.addAxiom(ClusterOntology, myAxiom);
	...
	private static HModel DataPropertyRangeAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		while (iterObj.hasNext()) {
			...
			int clusterId = BlockShareFunc.findClusterOfDro(obb, ontM);
			if (clusterId == -1) {
				...
				clusterId = BlockShareFunc.findClusterOfPro(ob, ontM);
			}
			if (clusterId == -1) {
				clusterId = BlockShareFunc.findClusterOfFuncPro(obb, ontM);
			}
			...
			if (clusterId == -1) {
				...
				while (allCl.hasNext()) {
					...
					while (onlyCl.hasNext()) {
						...
						clusterId = BlockShareFunc.findClusterOfClass(cc, ontM);
						if (clusterId != -1)
	...
	private static HModel SubObjectPropertyOfAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		int clusterIdSub = BlockShareFunc.findClusterOfPro(subProperty.asOWLObjectProperty(), ontM);
		int clusterIdSuper = BlockShareFunc.findClusterOfPro(superProperty.asOWLObjectProperty(), ontM);
		if (clusterIdSuper != -1) {
	...
	private static HModel InverseFunctionalObjectPropertyAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		while (iter.hasNext()) {
			...
			if (eqObj != null) {
				objList.add(eqObj);
				clId.add(BlockShareFunc.findClusterOfPro(eqObj, ontM));
			} else {
				objList.add(obj);
				clId.add(BlockShareFunc.findClusterOfPro(obj, ontM));
			}
		}
		boolean same = areSameId(clId);
		int clusterId = clId.get(0);
	...
	private static HModel InsatncesAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		if (cc instanceof OWLClass) {
			...
			if (refClass != null)
				cc = refClass;
			int clusterId = BlockShareFunc.findClusterOfClass(cc, ontM);
	...
	private static HModel EquivalentClassesAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		for (OWLClassExpression cls : ((OWLEquivalentClassesAxiom) myAxiom).getClassExpressions()) {
			if (cls instanceof OWLClassImpl) {
				...
				int clusterId = BlockShareFunc.findClusterOfClass(eqClass1, ontM);
				if (clusterId != -1) {
					ClusterManager = ontM.getClusters().get(clusterId).getManager();
					ClusterOntology = ontM.getClusters().get(clusterId).getOntology();
					ClusterManager.addAxiom(ClusterOntology, myAxiom);
				...
				while (clscls.hasNext()) {
					...
					if (refClass2 != null) {
						...
						clId.add(BlockShareFunc.findClusterOfClass(refClass2, ontM));
					} else {
						eqClass2.add(cl.asOWLClass());
						clId.add(BlockShareFunc.findClusterOfClass(cl.asOWLClass(), ontM));
					}
				}
				boolean same = areSameId(clId);
				int clusterId = -1;
	...
	private static HModel DisjointClassesAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		int clusterId = BlockShareFunc.findClusterOfClass(eqC1, ontM);
		int clusterIdSup = BlockShareFunc.findClusterOfClass(eqC2, ontM);
	...
	private static HModel DataPropertyDomainAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		if (dom instanceof OWLClassImpl) {
			...
			int clusterId = BlockShareFunc.findClusterOfClass(eqDom, ontM);
			...
			while (rangExist.hasNext()) {
				...
				if (refClass != null)
					eq = refClass;
				clId.add(BlockShareFunc.findClusterOfClass(eq, ontM));
			}
			boolean same = areSameId(clId);
			int clusterId = clId.get(0);
			if (same && clusterId != -1) {
	...
	private static HModel SubClassAdder(OWLAxiom myAxiom, HModel ontM) {
		...
		if (SuperClass instanceof OWLClassImpl && SubClass instanceof OWLClassImpl) {
			...
			int clusterIdSub = BlockShareFunc.findClusterOfClass(eqSub, ontM);
			int clusterIdSup = BlockShareFunc.findClusterOfClass(eqSup, ontM);
